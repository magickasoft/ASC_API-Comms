...
exports = module.exports = function(config) {
  ...
  // Log setup
  var debug = require('debug')(`${config.SRV_NAME}:router:email:template:delete`);

  return function(req, res) {
    EmailTemplate = EmailTemplate || mongoose.model('EmailTemplate');

    req.params = req.params || {};
    debug(`Deleting the email template ${req.params.id}`);
    ...
    if (req.params.id) {
      ...
      EmailTemplate.findOne(req.filterOwner, function(err, result) {
        if (err) {
